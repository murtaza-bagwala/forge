...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/forge-cli-');
    const execPath = path.join(root, '.claude/skills/forge/browse/dist/browse');
    const serverPath = path.join(root, '.claude/skills/forge/browse/src/server.ts');
