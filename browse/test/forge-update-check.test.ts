/**
 * Tests for bin/forge-update-check bash script.
 *
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'forge-update-check');
...
function run(extraEnv: Record<string, string> = {}) {
  const result = Bun.spawnSync(['bash', SCRIPT], {
    env: {
      ...
      GSTACK_REMOTE_URL: `file://${join(forgeDir, 'REMOTE_VERSION')}`,
      ...extraEnv,
    },
    stdout: 'pipe',
    stderr: 'pipe',
...
beforeEach(() => {
  forgeDir = mkdtempSync(join(tmpdir(), 'forge-upd-test-'));
  stateDir = mkdtempSync(join(tmpdir(), 'forge-state-test-'));
});
...
describe('forge-update-check', () => {
  ...
  test('exits 0 with no output when VERSION file is empty', () => {
    writeFileSync(join(forgeDir, 'VERSION'), '');
    const { exitCode, stdout } = run();
    expect(exitCode).toBe(0);
    expect(stdout).toBe('');
  });
  ...
  test('outputs JUST_UPGRADED and deletes marker', () => {
    writeFileSync(join(forgeDir, 'VERSION'), '0.4.0\n');
  ...
  test('exits silently when cache says UP_TO_DATE and is fresh', () => {
    writeFileSync(join(forgeDir, 'VERSION'), '0.3.3\n');
  ...
  test('re-checks when UP_TO_DATE cache version does not match local', () => {
    ...
    writeFileSync(join(forgeDir, 'REMOTE_VERSION'), '0.5.0\n');
  ...
  test('re-checks when local version does not match cached old version', () => {
    ...
    writeFileSync(join(forgeDir, 'REMOTE_VERSION'), '0.4.0\n');
  ...
  test('writes UP_TO_DATE cache when versions match', () => {
    writeFileSync(join(forgeDir, 'VERSION'), '0.3.3\n');
    writeFileSync(join(forgeDir, 'REMOTE_VERSION'), '0.3.3\n');
  ...
  test('treats invalid remote response as up to date', () => {
    writeFileSync(join(forgeDir, 'VERSION'), '0.3.3\n');
    writeFileSync(join(forgeDir, 'REMOTE_VERSION'), '<html>404 Not Found</html>\n');
  ...
  test('exits 0 with real project VERSION and unreachable remote', () => {
    ...
    writeFileSync(join(forgeDir, 'VERSION'), version + '\n');
