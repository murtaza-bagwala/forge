...
// Eval result collector — accumulates test results, writes to ~/.forge-dev/evals/ on finalize
...
function dumpOutcomeDiagnostic(dir: string, label: string, report: string, judgeResult: any) {
  try {
    const transcriptDir = path.join(dir, '.forge', 'test-transcripts');
...
Report whether it worked.`,
  ...
  test('SKILL.md setup block handles missing local binary gracefully', async () => {
    // Create a tmpdir with no browse binary — no local .claude/skills/forge/browse/dist/browse
...
Report the exact output. Do NOT try to fix or install anything — just report what you see.`,
    ...
    // On dev machines with forge installed globally, the fallback path
    // ~/.claude/skills/forge/browse/dist/browse exists, so we get READY.
