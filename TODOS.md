...
**What:** Add a `GSTACK_AUTO_UPGRADE=1` env var or `~/.forge/config` option that skips the AskUserQuestion prompt and upgrades automatically when a new version is detected.
...
**Context:** Requires `ANTHROPIC_API_KEY` in CI secrets. Cost is ~$4/run. The eval persistence system (v0.3.6) writes JSON to `~/.forge-dev/evals/` — CI would upload these as GitHub Actions artifacts and use `eval:compare` to post a delta comment on the PR.
...
**Context:** Reads the same `~/.forge-dev/evals/*.json` files. ~200 lines HTML + chart.js code served via a simple Bun HTTP server. No external dependencies beyond what's already installed.
