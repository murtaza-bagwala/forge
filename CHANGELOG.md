...
### Added
- **E2E observability** — heartbeat file (`~/.forge-dev/e2e-live.json`), per-run log directory (`~/.forge-dev/e2e-runs/{runId}/`), progress.log, per-test NDJSON transcripts, persistent failure transcripts. All I/O non-fatal.
...
- Stale servers from global install no longer shadow local changes — removed legacy `~/.claude/skills/forge` fallback from `resolveServerScript()`
...
### Changed
- State file location: `.forge/browse.json` (was `/tmp/browse-server.json`)
- Log files location: `.forge/browse-{console,network,dialog}.log` (was `/tmp/browse-*.log`)
...
- Legacy fallback to `~/.claude/skills/forge/browse/src/server.ts`
