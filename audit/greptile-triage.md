...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/forge/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
PROJECT_HISTORY="$HOME/.forge/projects/$REMOTE_SLUG/greptile-history.md"
```
...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/forge/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
mkdir -p "$HOME/.forge/projects/$REMOTE_SLUG"
mkdir -p ~/.forge
```

Append one line per triage outcome to **both** files (per-project for suppressions, global for retro):
- `~/.forge/projects/$REMOTE_SLUG/greptile-history.md` (per-project)
- `~/.forge/greptile-history.md` (global aggregate)
...
Example entries:
```
2026-03-13 | murtaza-bagwala/myapp | fp | app/services/auth_service.rb | race-condition
2026-03-13 | murtaza-bagwala/myapp | fix | app/models/user.rb | null-check
2026-03-13 | murtaza-bagwala/myapp | already-fixed | lib/payments.rb | error-handling
```
