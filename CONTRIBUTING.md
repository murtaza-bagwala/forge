...
Thanks for wanting to make forge better. Whether you're fixing a typo in a skill prompt or building an entirely new workflow, this guide will get you up and running fast.

## Quick start

forge skills are Markdown files that Claude Code discovers from a `skills/` directory. Normally they live at `~/.claude/skills/forge/` (your global install). But when you're developing forge itself, you want Claude Code to use the skills *in your working tree* — so edits take effect instantly without copying or deploying anything.
...
When E2E tests run, they produce machine-readable artifacts in `~/.forge-dev/`:
...
Artifacts are never cleaned up — they accumulate in `~/.forge-dev/` for post-mortem debugging and trend analysis.
...
- **Dev mode shadows your global install.** Project-local skills take priority over `~/.claude/skills/forge`. `bin/dev-teardown` restores the global one.
...
When you're developing forge in one workspace and want to test your branch in a
different project (e.g. testing browse changes against your real app), there are
two cases depending on how forge is installed in that project.

### Global install only (no `.claude/skills/forge/` in the project)
...
### Vendored project copy (`.claude/skills/forge/` checked into the project)

Some projects vendor forge by copying it into the repo (no `.git` inside the
...
2. **Replace the vendored copy** in the other project:
   ...
   cp -Rf ~/.claude/skills/forge .claude/skills/forge
   rm -rf .claude/skills/forge/.git

   # Rebuild binary and re-create skill symlinks
   cd .claude/skills/forge && ./setup
   ```
